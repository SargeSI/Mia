...
- **Ложные переключения на нейтральных сообщениях** («Привет», «Давай», «Как дела?»): см. четырёхслойную защиту ниже. Модуль `recall_router.py`, словарь `stop_words.json`. Фильтр: синтаксис → стоп-словарь → дисперсия эмбеддинга → штатный recall.
